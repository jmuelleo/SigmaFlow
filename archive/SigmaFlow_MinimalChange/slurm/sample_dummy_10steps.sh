#!/bin/bash -l
#
# Test 2 (fewer steps): re-sample the 10 dummy complexes with the SAME
# already-trained, unmodified checkpoint used for sampling_output_prodhparams
# (PAUSE-PUNKT #11 production-hyperparameter run), but with a much coarser
# ODE discretization (ode.num_steps=10 instead of the conf/sampling/base.yaml
# default of 25). No retraining, no code change - runs directly against the
# existing SigmaFlow_Development deployment on ARC.
#
# Purpose: after Test 1 (oracle vector field, see STATUS.md PAUSE-PUNKT #14)
# already showed that 25-step Euler integration reconstructs fragment
# junctions exactly given a PERFECT vector field, this checks whether the
# network's predicted (imperfect) field is unusually sensitive to step count
# in the direction that WORSENS junction bond lengths further - i.e. rules
# out/confirms "the current 25-step setting is itself part of the problem
# for the REAL (non-oracle) trajectory", independently from Test 1.
#
# Usage: sbatch slurm/sample_dummy_10steps.sh
# (CKPT_DIR/DATA_DIR/OUTPUT_DIR can still be overridden via env vars.)
#
#SBATCH --job-name=sigmaflow-sample-10steps
...
OUTPUT_DIR="${OUTPUT_DIR:-$(pwd)/sampling_output_10steps}"
...
$PYTHON scripts/sample.py \
    ...
    run_tag=dummy_overfit_check_10steps \
    num_seeds=1 \
...
echo "Ran with: ode.num_steps=10 (baseline was 25, see sampling_output_prodhparams)"
