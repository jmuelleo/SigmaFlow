...
Relative vector $r_{ij}=x_j-x_i$ & $r_{ij}\mapsto Qr_{ij}$ & equivariant \\
...
vectors $r_{ij}=x_j-x_i$, which satisfy
...
\item the edge direction $\hat r_{ij}=r_{ij}/\|r_{ij}\|\in S^2$ ---
...
made it one: $Y^{\ell_2}(\hat r_{ij})\in\RR^{2\ell_2+1}$ transforms by
...
\[
h_j^{(\ell_1)}\ \otimes_{\mathrm{cg}}\ Y^{\ell_2}(\hat r_{ij})
\ \longrightarrow\
...
\[
\begin{array}{ccc}
h_j^{(\ell_1)},\ r_{ij} & \Longrightarrow & \text{split } r_{ij}\text{ into } \|r_{ij}\| \text{ and } \hat r_{ij}\\[4pt]
& \Downarrow & \\[2pt]
\multicolumn{3}{c}{Y^{\ell_2}(\hat r_{ij})\ \text{(angular, steerable)}
\qquad\text{and}\qquad
\mathrm{RBF}(\|r_{ij}\|)\ \text{(radial, invariant)}}\\[4pt]
& \Downarrow & \\[2pt]
