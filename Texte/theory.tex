...
blocks are indexed by $m$, so capping $m$ caps the number of blocks --- and it
is a genuine approximation: the discarded components are not zero, they are
dropped. Worth knowing when reasoning about what angular patterns the model
can represent; the answer is not simply ``everything up to degree $3$''.
\end{remark}
...
$m_{\max}<L$ truncation & \textbf{approximate} & discards $|m|>m_{\max}$ \\
\bottomrule
\end{tabular}
\end{center}

Honest summary: every operation is exactly equivariant \emph{except} the
nonlinearity and the $m$-truncation, and both are approximations whose size
grows with $L$. ``Equivariant by construction'' is true of the
architecture's design and false of its arithmetic.
\end{tcolorbox}
