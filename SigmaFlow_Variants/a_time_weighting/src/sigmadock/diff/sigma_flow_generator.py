...
class SigmaFlowGenerator(nn.Module):
    ...
    def _time_weight(self, t: torch.Tensor, cap_t: float = 0.9) -> torch.Tensor:
        """
        t:[B x F] time value per fragment
        we want to stronger weigh the loss near the end of the sampling, to reduce bond lengths!
        Explorative <- is an idea how this might be fixed as diffusion brings a time weighting componant naturally, while here we don't have that <- sigmadock doesnt have the issues, but has same Equiformer
        is somewhat inspired by Yim et al. 2023, who clip (1/(1-t)^2) for their parametrisation
        """
    ...
    def compute_losses(self, out: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        weight = self._time_weight(t_batch)
        #  ============ SCORE LOSS ============ #
        pt, tt = out["pred_u_t_trans"], out["u_t_trans"]
        batch_trans_vector_field_loss = weight*(
            ((pt - tt).pow(2).sum(-1))
        )  # [B x F]
        ...
        err = pr - tr
        batch_rot_vector_field_loss = weight*(err * err).sum(dim=(-1, -2))  # [B x F]
