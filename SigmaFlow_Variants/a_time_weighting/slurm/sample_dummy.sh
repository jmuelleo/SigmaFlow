...
# See train_dummy_overfit_gpu_3h_time_weighting.sh: sigmaflow_env's editable
# `sigmadock` install points at SigmaFlow_Development/src by default, so
# this override is required here too, or sampling would silently use the
# unmodified baseline code instead of this variant.
export PYTHONPATH="$(pwd)/src:${PYTHONPATH:-}"
CKPT_DIR="${CKPT_DIR:?Set CKPT_DIR to your checkpoint path, e.g. experiments/sigmadock/<timestamp>/checkpoints/last.ckpt}"
DATA_DIR="${DATA_DIR:-/data/stat-cadd/shug8458/SigmaFlow_Development_JulianMueller/SigmaFlow/SigmaFlow_Variants/a_time_weighting/notebooks}"
...
# Sanity check: must print a path under this job's own
# SigmaFlow_Variants/a_time_weighting/src/, not SigmaFlow_Development/.
