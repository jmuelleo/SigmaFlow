#!/bin/bash -l
#
# 6h pdbbind-general run: VARIANTE B (rotation data-space loss) + FRAME FIX
#
# WHAT THIS VARIANT CHANGES
# Adds a data-space term on rotations: a single Euler jump from R_t to t=1
# gives R_1_hat = R_t @ exp(pred_u_t_R * (1-t)), compared against the true
# R_1 via log(R_1_hat^T R_1).
#
# RELATION TO THE FRAME FIX: variant B was MIS-FRAMED TWICE. Its jump
# right-multiplies (R_t @ exp(...)), i.e. it already assumes pred_u_t_R is a
# BODY-frame quantity - which it was not. So both the base loss AND this
# variant's own construction were built on a world-frame vector. The fix
# repairs both at once; the variant's own code needed NO change.
#
# Previous (mis-framed) run: job 8490675, 12h - no effect.
#
...
#
# Usage: sbatch slurm/train_pdbbind_general_6h_framefix_b.sh
#
#SBATCH --job-name=sigmaflow-b-rotdata-framefix-6h
...
echo "Ran: 6h pdbbind-general, VARIANTE B (rotation data-space loss) + FRAME FIX, LR-fixed (max_epochs=3)"
