...
class SigmaFlowGenerator(nn.Module):
    ...
    def compute_losses(self, out: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        """
        Compute the core vector-field losses, plus a rotation data-space
        auxiliary term:
          - Translation vector_field MSE
          - Rotation vector_field MSE
          - Rotation data-space term (added into loss_R): compares the
            true final rotation R_1 against the network's implied estimate
            R_1_hat, obtained by a single Euler jump from R_t straight to
            t=1 using the predicted velocity (dt=1-t) — mirrors SigmaDock's
            R0 data-space loss (denoiser.py::compute_losses), but built
            from SigmaFlow's own exp-map step (so3_flow_matcher.py::
            euler_step) instead of a reverse-SDE step, since there is no
            SDE here. Not added for translation: for the linear/CondOT
            path used here, an analogous translation data-space term is
            an EXACT scalar rescaling of the existing translation loss
            (see STATUS.md "Variante (b) verworfen" and "Praezisierung:
            was T0/R0 genau vergleichen" for the derivation) - i.e. no new
            information, so it is deliberately omitted.
        ...
        #  ============ ROTATION DATA-SPACE LOSS (new) ============ #
        R_t, R_1 = out["R_t"], out["R_1"]  # [B x F, 3, 3] each
        dt_to_data = (1.0 - t_batch)[:, None, None]  # [B x F, 1, 1]
        R_1_hat = R_t @ so3_utils.exp(pr * dt_to_data)  # single jump straight to t=1
        log_rel = so3_utils.log(R_1_hat.transpose(-1, -2) @ R_1)  # [B x F, 3, 3]
        batch_rot_data_space_loss = (log_rel * log_rel).sum(dim=(-1, -2))  # [B x F]

        return {
            # Batch per-fragment loss
            "loss_trans": batch_trans_vector_field_loss,  # [B x F]
            "loss_R": batch_rot_vector_field_loss + batch_rot_data_space_loss,  # [B x F]
        }
