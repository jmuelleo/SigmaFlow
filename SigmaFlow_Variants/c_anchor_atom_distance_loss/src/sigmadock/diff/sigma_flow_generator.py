...
class SigmaFlowGenerator(nn.Module):
    ...
    def get_transformations_from_rototranslations(
        ...
        Args:
            ...
            trans_1:     [F_total,3]  concatenated COMs for all fragments in batch order.
        ...
        New pos[i] = R[f_global] @ (pos_old[i] - trans_1[f_global]) + trans_1[f_global] + delta_T[f_global]
        ...
        trans_1_g = trans_1[f_global]  # [M,3]
        dT_g = delta_T[f_global]  # [M,3]

        pts = pos0[is_lig]  # [M,3]
        rel = pts - trans_1_g  # [M,3]

        # apply rotation and translation
        rel_rot = torch.matmul(R_g, rel.unsqueeze(-1)).squeeze(-1)  # [M,3]
        new_pts = rel_rot + trans_1_g + dT_g  # [M,3]
    ...
    def _sample_time(self, batch: Batch | Data) -> tuple[torch.Tensor, torch.Tensor, dict[str, torch.Tensor]]:
        """
    ...
    def _get_initial_states(self, batch: Batch | Data) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        return (
            pos_0,
            torch.cat(trans_1_list, dim=0),
            torch.cat(R_1_list, dim=0),
            num_fragments,
        )
    ...
    def _sample_flow(
        ...
        """
        Sample forward conditonal probability path.

        Args:
            trans_1 (torch.Tensor): Initial fragment centers [B*F,3].
            R_1 (torch.Tensor): Initial rotations [B*F,3,3].
            t (torch.Tensor): Times per fragment [BxF].

        Returns:
            sampled (dict): Contains 'trans_t', 'R_t', 'u_t_trans', 'u_t_R'.
        """
        sampled = self.flow_matcher.conditional_probability_path(trans_1 = trans_1, R_1 = R_1, t = t_batch)
        result = {"trans_t": sampled["trans_t"],
                  "R_t": sampled["R_t"],
                  "u_t_trans": sampled["u_t_trans"],
                  "u_t_R": sampled["u_t_R"]}
        return result
    ...
    def _compute_anchor_distance_loss(
        self,
        batch: Batch,
        pos_0: torch.Tensor,  # [N,3] true atom positions
        pos_1_hat: torch.Tensor,  # [N,3] single-jump predicted final atom positions
        num_fragments: torch.Tensor,  # [B]
    ) -> torch.Tensor:
        """
        Direct anchor-atom distance loss across each cut torsional bond.

        Every fragmentation cut leaves a pair of "anchor" atoms (one per
        fragment) that used to be bonded. Both fragments are placed
        independently by the model, so nothing constrains this specific
        distance the way ordinary bond lengths are constrained by rigid
        per-fragment geometry - this is exactly the "transition bond" gap
        measured externally with RDKit throughout STATUS.md. This loss
        penalises that distance directly, instead of only an indirect proxy
        (a reweighted SO(3)/R3 velocity error), which is what the two prior
        variants (a_time_weighting, b_rotation_data_space_loss) did and
        which showed no effect.

        The anchor-anchor edges already exist in the ligand graph (added in
        get_global_ligand_graph, chem/processing.py, as part of the
        triangulation-conditioning edges) with edge type
        "ligand_torsional_bond" - both directions are present, we keep one.

        Args:
            batch: Batch with edge_index/edge_entity (ligand_torsional_bond
                edges) and frag_idx_map (local fragment id per atom, -1 for
                protein atoms).
            pos_0: True atom positions (see forward(); pos_0 already IS the
                reference/bound structure here, despite the name).
            pos_1_hat: Predicted final atom positions, reconstructed by
                applying the single-jump predicted (R_1_hat, trans_1_hat)
                per fragment via _apply_transformations - same mechanism
                used for pos_t, just with the predicted end-state instead of
                the sampled noisy state.
            num_fragments: Fragments per complex, to size the [B x F] output
                so it is compatible with the other per-fragment losses in
                compute_losses()/scaled_fragmented_loss().

        Returns:
            torch.Tensor: [B x F] per-fragment loss (each bond's squared
            distance error is added into ONE of its two fragments' slots -
            scaled_fragmented_loss() immediately sums over all fragments of
            a complex, so which of the two slots holds it does not matter).
        """
        total_fragments = int(num_fragments.sum().item())
        device = pos_0.device

        torsional_edge_type = HPARAMS.get_edge_idx("ligand_torsional_bond")
        is_torsional = batch.edge_entity == torsional_edge_type
        u, v = batch.edge_index[0, is_torsional], batch.edge_index[1, is_torsional]
        # These edges were added in both directions (u->v and v->u); keep one per bond.
        keep = u < v
        u, v = u[keep], v[keep]

        if u.numel() == 0:
            return torch.zeros(total_fragments, device=device, dtype=pos_0.dtype)

        true_dist = (pos_0[u] - pos_0[v]).norm(dim=-1)  # [T]
        pred_dist = (pos_1_hat[u] - pos_1_hat[v]).norm(dim=-1)  # [T]
        bond_loss = (pred_dist - true_dist).pow(2)  # [T]

        # Global fragment index (0..total_fragments-1) of atom u's fragment,
        # same offset construction as get_transformations_from_rototranslations.
        frag_local = batch.frag_idx_map
        sample_idx = batch.batch if hasattr(batch, "batch") else torch.zeros_like(frag_local)
        offsets = torch.cumsum(
            torch.cat([torch.zeros(1, device=device, dtype=num_fragments.dtype), num_fragments[:-1]]),
            dim=0,
        )
        f_global_u = offsets[sample_idx[u]] + frag_local[u]  # [T]

        anchor_dist_loss = torch.zeros(total_fragments, device=device, dtype=bond_loss.dtype)
        anchor_dist_loss.index_add_(0, f_global_u, bond_loss)
        return anchor_dist_loss
    ...
    def _compute_fragment_dynamics(
        ...
        # Mass & inertia at trans_1, then at time t
        ...
        force_per_fragment, torque_per_fragment = self.linear_mechanics(
            ...
            coms=trans_t,  # [B x F, 3] fragment centers at time t
        )  # [B x F, 3], [B x F, 3]
    ...
    def _compute_vector_field(self,
                              ...
                              t_batch: torch.Tensor) -> dict[str, torch.Tensor]:
        """
        Interprets the pooled per-fragment force/torque as the predicted conditional vector field.
        No time dependent scaling needed, unlike the diffusion score.

        FRAME FIX (see STATUS.md "ROOT CAUSE GEFUNDEN"):
        The rotational output of the prediction head lives in the WORLD frame -
        newton_maruyama builds it as `omega = hat(dW_world)` and its own comment
        says "Transport into GLOBAL (left-invariant - WORLD) frame of reference".
        That convention is inherited from SigmaDock, which consumes it
        consistently by LEFT-multiplying (`dR @ R_t`, denoiser.py).

        SigmaFlow, however, is formulated in the BODY frame throughout:
          - target:      calc_rot_vector_field -> log(R_t^T @ R_1)/(1-t),
                         documented as "right trivialised vector field"
          - integration: euler_step -> R_next = R_t @ exp(v_t*dt)   (RIGHT mult.)

        Previously `pred_u_t_R = updates["omega"]` handed the world-frame
        quantity straight into a body-frame loss and a body-frame ODE, with no
        conversion. The adjoint transport below closes that gap:

            R_t @ exp(u_body*dt) = exp(w_world*dt) @ R_t
          <=>          u_body    = R_t^T @ w_world @ R_t

        Conjugating a skew-symmetric matrix by a rotation keeps it skew-symmetric,
        so pred_u_t_R stays a valid so(3) element and every downstream shape is
        unchanged. Translation needs no such transport: total_force is an R^3
        vector in the world frame and u_t_trans is defined in the same frame.
        """
        R_t = sampled["R_t"]  # [B*F,3,3]
        pred_u_t_trans = updates["total_force"] # [B*F,3]
        # World-frame -> body-frame (right-trivialised), matching target & euler_step.
        pred_u_t_R = R_t.transpose(-1, -2) @ updates["omega"] @ R_t  # [B*F,3,3]
        result = {"pred_u_t_trans": pred_u_t_trans,
                  "pred_u_t_R": pred_u_t_R}
        return result
    ...
    def _compute_true_vector_field(
        self,
        trans_1: torch.Tensor,
        R_1: torch.Tensor,
        Tt: torch.Tensor,
        ...
        """
        Compute true vector_field for translation and rotation based on the batch data.
    ...
    def forward(
        ...
        **model_kwargs: dict[str, Any],
        ...
        # Compute translational and rotational vector_field
        vector_field = self._compute_vector_field(sampled_flow, fragment_updates, t_batch)

        # Single-jump reconstruction of the predicted final state (t=1), reusing the
        # same CondOT/exp-map step _apply_transformations already uses for pos_t -
        # just with the predicted end-state (R_1_hat, trans_1_hat) instead of the
        # sampled noisy state (R_t, trans_t). Needed for the anchor-distance loss.
        dt_to_data = (1.0 - t_batch)[:, None]  # [B x F, 1]
        trans_1_hat = sampled_flow["trans_t"] + vector_field["pred_u_t_trans"] * dt_to_data  # [B x F, 3]
        R_1_hat = sampled_flow["R_t"] @ so3_utils.exp(vector_field["pred_u_t_R"] * dt_to_data[:, :, None])  # [B x F, 3, 3]
        pos_1_hat = self._apply_transformations(
            pos_0=pos_0,
            batch=batch,
            trans_1=trans_1,
            R_1=R_1,
        ...
        anchor_dist_loss = self._compute_anchor_distance_loss(
            batch=batch, pos_0=pos_0, pos_1_hat=pos_1_hat, num_fragments=num_fragments
        )  # [B x F]
        ...
        return {
            ...
            "u_t_trans": sampled_flow["u_t_trans"],  # [B x F, 3]
            "u_t_R": sampled_flow["u_t_R"],  # [B x F, 3, 3]
            "anchor_dist_loss": anchor_dist_loss,  # [B x F]
    ...
    def compute_losses(self, out: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        Compute the four core losses:
          - Translation vector_field MSE
          - Rotation vector_field MSE
          - Anchor-atom distance loss (new, added directly to loss_trans - see
            _compute_anchor_distance_loss for the construction. Optional here:
            out["anchor_dist_loss"] is only present when this dict comes from
            forward()'s training path, not from sampling.py's hand-built
            per-step logging dicts, which only track the velocity-field terms.)
        ...
        #  ============ SCORE LOSS ============ #
        pt, tt = out["pred_u_t_trans"], out["u_t_trans"]
        batch_trans_vector_field_loss = (
            ((pt - tt).pow(2).sum(-1))
        )  # [B x F]

        pr, tr = out["pred_u_t_R"], out["u_t_R"]
        # Frobenius norm of the Lie algebra element disentangles S (local rot matx from R_1' = R_1 @ S).
        err = pr - tr
        batch_rot_vector_field_loss = (err * err).sum(dim=(-1, -2))  # [B x F]
        # batch_rot_vector_field_loss = torch.linalg.norm(err, "fro", dim=(-1, -2)) ** 2  # [B x F]

        #  ============ ANCHOR-ATOM DISTANCE LOSS (new) ============ #
        anchor_dist_loss = out.get("anchor_dist_loss", 0.0)  # [B x F] or scalar 0.0 if absent

        return {
            # Batch per-fragment loss
            "loss_trans": batch_trans_vector_field_loss + anchor_dist_loss,  # [B x F]
            "loss_R": batch_rot_vector_field_loss,  # [B x F]
        }
