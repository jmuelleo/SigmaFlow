...
def main() -> None:  # noqa: C901
    ...
    try:
        if args.resume_from_checkpoint and "ckpt_path" in locals():
            ...
            # refuses to unpickle our own checkpoint's custom classes
            # (sigmadock.oracle.HParams, pathlib.PosixPath) when restoring
            # full trainer state (optimizer, epoch) to resume training.
            # Safe to disable here: this is always our own, trusted
            # checkpoint, never an untrusted external file.
            trainer.fit(lightning_model, datamodule=datamodule, ckpt_path=ckpt_path, weights_only=False)
        else:
