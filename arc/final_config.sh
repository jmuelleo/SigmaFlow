...
EOF
    # Der Originalwert ist 64. conf/training/slurm.yaml setzt batch_size 64,
    # und config.py teilt den Wert durch world_size -- er ist also global, nicht
    # pro Karte. Die frueheren Laeufe mit effektiv 32 lagen darunter.
    if [ "${FINAL_EFFECTIVE_BATCH}" -ne 64 ]; then
        echo "[final_config] WARNUNG: effektive Batch ${FINAL_EFFECTIVE_BATCH} != 64 (Originalwert, global)."
    fi
}
