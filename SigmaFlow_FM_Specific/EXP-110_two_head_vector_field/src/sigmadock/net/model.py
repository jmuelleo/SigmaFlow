...
class EquiformerV2(nn.Module):
    def __init__(  # noqa: C901
        ...
        **kwargs: dict,
        ...
        # ------------------------------------------------------------------
        # EXP-110: ZWEI KOEPFE STATT EINEM
        #
        # SigmaFlow-Minimal hat genau einen l=1-Ausgang, den es als
        # Pseudo-KRAFT je Atom liest und anschliessend per Newton-Euler in
        # Translation und Rotation zerlegt. Diese Variante gibt stattdessen
        # ZWEI aequivariante l=1-Felder aus:
        #
        #   trans_block -> f_i^T   Beitrag zur Fragment-Translationsgeschwindigkeit
        #   rot_block   -> f_i^R   Beitrag zur Fragment-Winkelgeschwindigkeit
        #
        # Der Rumpf bleibt vollstaendig geteilt; getrennt sind nur die beiden
        # letzten Attention-Bloecke. Die Konfiguration beider Koepfe ist
        # zeichengleich, damit der Vergleich kontrolliert bleibt.
        # ------------------------------------------------------------------
        self.trans_block = SO2EquivariantGraphAttention(
            self.sphere_channels,
            self.attn_hidden_channels,
            self.num_heads,
            self.attn_alpha_channels,
        ...
        self.rot_block = SO2EquivariantGraphAttention(
            self.sphere_channels,
            self.attn_hidden_channels,
            self.num_heads,
            self.attn_alpha_channels,
    ...
    def forward(self, data: Batch, t: torch.Tensor, **kwargs: dict) -> None:
        ...
        # Zwei getrennte l=1-Ausgaenge auf derselben Rumpfdarstellung x.
        # narrow(1, 1, 3) greift die Koeffizienten l=1 heraus (Index 0 waere
        # der Skalar l=0, 4..8 waeren l=2).
        trans_field = self.trans_block(x, edge_embedding, data.edge_index, data.edge_entity)
        trans_field = trans_field.embedding.narrow(1, 1, 3).view(-1, 3)  # [N,3]

        rot_field = self.rot_block(x, edge_embedding, data.edge_index, data.edge_entity)
        rot_field = rot_field.embedding.narrow(1, 1, 3).view(-1, 3)      # [N,3]

        return trans_field, rot_field
