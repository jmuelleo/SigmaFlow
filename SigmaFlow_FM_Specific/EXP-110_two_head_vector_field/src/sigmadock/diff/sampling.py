...
def sample_notebook(
    ...
    def _predict_vector_field_step(
        ...
        # EXP-110: zwei Koepfe -> Mittel-Pooling. Exakt dieselbe Kette wie im
        # Training (SigmaFlowGenerator.forward), damit Inferenz und Training
        # nicht auseinanderlaufen.
        lig_trans_field, lig_rot_field, field_idxs = denoiser._compute_forces(
            batch=batch,
            t=torch.tensor([t] * batch.num_graphs, device=pos_t.device),  # [B]
        )  # je [B x NL, 3]

        flat_frag_idx = denoiser.get_flat_fragment_index(batch)              # [N]
        frag_idx_continuous = flat_frag_idx.index_select(0, field_idxs)      # [B x NL]

        v_frag, omega_frag = denoiser.pool_fragment_fields(
            trans_field=lig_trans_field,
            rot_field=lig_rot_field,
            frag_idx_continuous=frag_idx_continuous,
            num_fragments=int(R_t.shape[0]),
        )  # je [B x F, 3]

        # Compute [R3, so3] vector_field
        pred_vector_field = denoiser._compute_vector_field(
            {"R_t": R_t, "trans_t": trans_t},
            {"v_frag": v_frag, "omega_frag": omega_frag},
            t_batch,
        )
