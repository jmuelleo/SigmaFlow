...
class SigmaFlowGenerator(nn.Module):
    ...
    def _compute_forces(
        ...
        **model_kwargs: dict[str, Any],
        ...
        """
        Run model to get the two per-atom equivariant fields at non-masked nodes.
        ...
        # EXP-110: das Netz liefert ZWEI aequivariante l=1-Felder je Knoten.
        # Kein Vorzeichenwechsel mehr: in Minimal drehte `-epsilon_theta` die
        # eps-Vorhersage in eine "Kraft". Hier sagt der Kopf direkt einen
        # Geschwindigkeitsbeitrag vorher, es gibt also nichts umzudeuten.
        trans_field, rot_field = self.model(batch, t, **model_kwargs)  # je [N,3]
        field_idxs = get_lig_idxs(batch.node_entity, mask=batch.mask)  # [B x NL]
        lig_trans = trans_field.index_select(0, field_idxs)  # [B x NL, 3]
        lig_rot = rot_field.index_select(0, field_idxs)      # [B x NL, 3]
        assert batch.mask[field_idxs].all() and lig_trans.shape[0] == field_idxs.shape[0], (
            "Atomfelder sind nicht an den nicht maskierten Knoten ausgewaehlt."
        )
        return lig_trans, lig_rot, field_idxs

    @staticmethod
    def pool_fragment_fields(
        trans_field: torch.Tensor,      # [K,3] je Atom
        rot_field: torch.Tensor,        # [K,3] je Atom
        frag_idx_continuous: torch.Tensor,  # [K]
        num_fragments: int,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """Mittelt die Atomfelder je Fragment zu einer Geschwindigkeit und einer
        Winkelgeschwindigkeit.

        WARUM MITTEL UND NICHT SUMME
            Fragmente haben zwischen 1 und rund 30 Atomen. Eine Summe skaliert
            mit der Fragmentgroesse, das Ziel u_t aber nicht -- das Netz muesste
            die Groessenabhaengigkeit auslernen. Das Mittel ist
            groessenunabhaengig.

            Nebenbei ist das Mittel exakt das, was SigmaFlow-Minimal fuer die
            Translation ohnehin tut: dort ist dT = sum(f_i) / m_f mit
            m_f = Atomzahl, also ebenfalls ein Mittel. Die Translation aendert
            sich in dieser Variante also NICHT in der Form, nur in der Quelle
            (eigener Kopf statt geteiltem).

        WARUM DAS FUER DIE ROTATION ZULAESSIG IST
            Unter einer globalen Rotation Q transformiert ein l=1-Feld als
            f_i -> Q f_i, also mean(f_i) -> Q mean(f_i). Die gemittelte Groesse
            ist damit ein aequivarianter Weltrahmen-Vektor -- genau wie das
            Drehmoment in Minimal. Beide brauchen anschliessend dieselbe
            adjungierte Konjugation, weil das Ziel log(R_t^T R_1) INVARIANT
            ist. Der Rahmenwechsel ist also keine Eigenschaft des
            Drehmomentmechanismus, sondern folgt aus Aequivarianz gegen
            Invarianz.

            Unter SO(3) allein sind polare und axiale Vektoren
            ununterscheidbar; EquiformerV2 verfolgt keine Paritaet. Die
            Unterscheidung Kraft/Drehmoment ist hier daher ohne Konsequenz.

        Returns:
            v_frag:     [M,3] Fragment-Translationsgeschwindigkeit
            omega_frag: [M,3] Fragment-Winkelgeschwindigkeit, Weltrahmen
        """
        device, dtype = trans_field.device, trans_field.dtype
        counts = torch.zeros(num_fragments, device=device, dtype=dtype)
        counts.index_add_(0, frag_idx_continuous,
                          torch.ones_like(frag_idx_continuous, dtype=dtype))
        counts = counts.clamp_min(1.0)   # leere Fragmente kommen nicht vor; schuetzt vor 0/0

        v_frag = torch.zeros((num_fragments, 3), device=device, dtype=dtype)
        w_frag = torch.zeros((num_fragments, 3), device=device, dtype=dtype)
        v_frag.index_add_(0, frag_idx_continuous, trans_field)
        w_frag.index_add_(0, frag_idx_continuous, rot_field)

        return v_frag / counts[:, None], w_frag / counts[:, None]
    ...
    def _compute_vector_field(self,
        ...
        """Deutet die gepoolten Fragmentgroessen als vorhergesagtes Vektorfeld.

        TRANSLATION
            Der Zielrahmen ist der Weltrahmen (u_t_trans = (T_1 - T_t)/(1-t)),
            die gepoolte Geschwindigkeit ebenfalls. Keine Umrechnung.

        ROTATION -- HIER SITZT DIE EINZIGE RAHMENUMRECHNUNG
            Das Ziel ist log(R_t^T R_1)/(1-t). Unter einer globalen Rotation Q
            gilt (Q R_t)^T (Q R_1) = R_t^T R_1: das Ziel ist INVARIANT.
            Jede aequivariante Netzausgabe transformiert dagegen als Q(.)Q^T.
            Ein roher Ausgang kann das Ziel also nie treffen.

            Die adjungierte Konjugation schliesst genau diese Luecke:

                R_t exp(u_body dt) = exp(w_world dt) R_t
              <=>        u_body    = R_t^T w_world R_t

            und ist unter Q nachweislich invariant. Konjugation mit einer
            Rotation erhaelt die Schiefsymmetrie, u_body bleibt also in so(3).

            Das gilt fuer JEDE aequivariante Konstruktion, auch fuer das
            Drehmoment in SigmaFlow-Minimal. Der Frame-Fix ist kein Artefakt
            des Newton-Euler-Wegs.
        """
        R_t = sampled["R_t"]                       # [B*F,3,3]
        pred_u_t_trans = pooled["v_frag"]          # [B*F,3], Weltrahmen
        omega_world = so3_utils.hat(pooled["omega_frag"])   # [B*F,3,3], so(3)
        pred_u_t_R = R_t.transpose(-1, -2) @ omega_world @ R_t   # Koerperrahmen
        return {"pred_u_t_trans": pred_u_t_trans,
    ...
    def forward(
        ...
        **model_kwargs: dict[str, Any],
        ...
        # Zwei Atomfelder aus den getrennten Koepfen
        lig_trans_field, lig_rot_field, field_idxs = self._compute_forces(
            batch=batch, t=t, **model_kwargs
        )  # je [B x NL, 3]

        # Fragmentzuordnung der ausgewaehlten Atome
        flat_frag_idx = self.get_flat_fragment_index(batch)      # [N]
        frag_idx_continuous = flat_frag_idx.index_select(0, field_idxs)  # [B x NL]
        assert (frag_idx_continuous >= 0).all(), "Nicht-Ligandatom im Feldauswahlindex."

        # Mittel-Pooling je Fragment -> Geschwindigkeit und Winkelgeschwindigkeit
        v_frag, omega_frag = self.pool_fragment_fields(
            trans_field=lig_trans_field,
            rot_field=lig_rot_field,
            frag_idx_continuous=frag_idx_continuous,
            num_fragments=int(sampled_flow["R_t"].shape[0]),
        )  # je [B x F, 3]
        pooled = {"v_frag": v_frag, "omega_frag": omega_frag}

        # Compute translational and rotational vector_field
        vector_field = self._compute_vector_field(sampled_flow, pooled, t_batch)
        ...
        return {
            ...
            "R_1": R_1,  # [B x F, 3, 3]
            # Model Preds
            "atom_trans_field": lig_trans_field,  # [B x F x A, 3]
    ...
    def compute_losses(self, out: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        return {
            ...
            "loss_trans": batch_trans_vector_field_loss,  # [B x F]
