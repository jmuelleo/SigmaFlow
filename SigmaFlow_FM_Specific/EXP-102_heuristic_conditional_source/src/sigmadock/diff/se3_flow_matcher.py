...
class SE3_FlowMatcher:
    ...
    def sample_init(self, n: int, device: str,
                    ...
                    median_deg: float | None = None) -> dict[str, torch.Tensor]:
        trans_0 = self._r3_flow_matcher.sample_init(n, device)
        R_0 = self._so3_flow_matcher.sample_init(n, device, R_centre, median_deg)
    ...
    def conditional_probability_path(self, trans_1: torch.Tensor, R_1: torch.Tensor, t: torch.Tensor,
                                     R_centre: torch.Tensor | None = None,
                                     median_deg: float | None = None) -> dict[str, torch.Tensor]:
        # EXP-102: R_centre/median_deg steuern NUR die Rotationsquelle.
        # Die Translationsquelle bleibt unveraendert N(0, I) - eine zweite
        # Aenderung waere ein Confounder.
        trans_t, u_t_trans = self._r3_flow_matcher.conditional_probability_path(trans_1, t)
        R_t, u_t_R = self._so3_flow_matcher.conditional_probability_path(R_1, t, R_centre, median_deg)
