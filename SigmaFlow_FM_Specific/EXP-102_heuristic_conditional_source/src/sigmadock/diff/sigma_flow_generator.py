...
class SigmaFlowGenerator(nn.Module):
    ...
    def __init__(
        ...
        # EXP-102: Quellverteilung auf SO(3). Die EINZIGE Aenderung des Experiments.
        #   "haar"       -> bit-identisch zu EXP-100 (Kontrollbedingung)
        #   "pocket_pca" -> Zentrum aus Hauptachsen Konformer -> Tasche
        #   "constant"   -> festes Zentrum, misst den Effekt der KONZENTRATION
        #                   ohne jede Konditionierung
        source_mode: str = "haar",
        # Medianwinkel der Quelle zum Zentrum, in Grad. Haar hat 132.3.
        # Wird aus arc/exp101_distance_audit.py gesetzt, nicht geraten.
        source_median_deg: float = 132.3,
        # Nur fuer source_mode="constant": [3,3], z.B. das Karcher-Mittel der
        # Zielrotationen ueber den TRAININGSsplit.
        source_constant: torch.Tensor | None = None,
        # Misc
        verbose: bool = False,
        ...
        **kwargs: Any,
        ...
        # --- EXP-102 -------------------------------------------------------
        if source_mode not in ("haar", "pocket_pca", "constant"):
            raise ValueError(f"source_mode unbekannt: {source_mode}")
        self.source_mode = source_mode
        self.source_median_deg = float(source_median_deg)
        if source_mode == "constant":
            if source_constant is None:
                raise ValueError("source_mode='constant' braucht source_constant [3,3].")
            self.register_buffer("source_constant", torch.as_tensor(source_constant, dtype=torch.float32))
        else:
            self.source_constant = None
        # Eine konzentrierte Quelle senkt die Diversitaet. Die groesste
        # gemessene Luecke des Projekts ist aber genau dort (ein Zug 1.9 %,
        # best-of-10 29.2 %). Deshalb der Hinweis im Log statt stiller Annahme.
        if source_mode != "haar" and self.source_median_deg < 60.0:
            import warnings
            warnings.warn(
                f"[EXP-102] Quelle stark konzentriert (Median {self.source_median_deg:.0f} Grad "
                f"gegen Haar 132.3). Oracle@K MUSS mitberichtet werden - eine bessere "
                f"Einzelziehung bei schlechterem Oracle@K waere ein Nettoverlust.",
                stacklevel=2)
    ...
    def source_centres(self, batch, num_fragments: torch.Tensor) -> torch.Tensor | None:
        """EXP-102: Zentrumsrotation je Fragment, oder None fuer Haar.

        Alle Fragmente EINES Komplexes bekommen dieselbe Rotation. Das ist
        keine Vereinfachung, sondern die einzige Moeglichkeit: bei t = 0 haben
        die Fragmente noch keine Position, auf die man konditionieren koennte.

        Leakage-frei nach Konstruktion: `ref_conf_pos` ist der ETKDG-Konformer
        im EIGENEN Frame, `ref_pos[prot]` die Taschenatome. Weder die
        Kristallpose des Liganden noch `pos_0` (das die Fragmente an den
        Kristall-COM verschiebt) gehen ein.
        """
        if self.source_mode == "haar":
            return None
        from sigmadock.diff.conditional_source import pocket_alignment_rotation
        if not hasattr(batch, "ref_conf_pos") or batch.ref_conf_pos is None:
            raise ValueError("[EXP-102] ref_conf_pos fehlt; ohne den leakage-freien "
                             "Konformer ist die Heuristik nicht definierbar.")
        ptr = batch.ptr if hasattr(batch, "ptr") else torch.tensor([0, batch.x.shape[0]])
        lig_virt = HPARAMS.get_node_idx("ligand_virtual")
        centres = []
        for i in range(len(ptr) - 1):
            a, b = int(ptr[i]), int(ptr[i + 1])
            fm, ne, m = batch.frag_idx_map[a:b], batch.node_entity[a:b], batch.mask[a:b]
            lig = (fm >= 0) & (ne != lig_virt) & m
            prot = fm < 0
            if self.source_mode == "constant":
                Q = self.source_constant.to(batch.ref_pos.device)
            else:                                    # "pocket_pca"
                Q = pocket_alignment_rotation(batch.ref_conf_pos[a:b][lig],
                                              batch.ref_pos[a:b][prot])
            centres.append(Q.expand(int(num_fragments[i]), 3, 3))
        return torch.cat(centres, dim=0) if centres else None
    ...
    def _sample_flow(
        ...
        """
        sampled = self.flow_matcher.conditional_probability_path(
            trans_1 = trans_1, R_1 = R_1, t = t_batch,
            R_centre = R_centre, median_deg = self.source_median_deg)
    ...
    def forward(
        ...
        **model_kwargs: dict[str, Any],
        ...
        sampled_flow = self._sample_flow(trans_1 = trans_1,
                                         R_1 = R_1,
                                         t_batch = t_batch,
                                         R_centre = self.source_centres(batch, num_fragments))
