...
class SigmaLightningModule(pl.LightningModule):
    ...
    def _shared_step(
        ...
        total_loss = (
            self.trans_score_weight * losses["loss_trans"]
        ...
        log_dict: dict[str, float | torch.Tensor] = {
            ...
            f"loss_{stage}/loss_trans": losses["loss_trans"].mean(),
            f"loss_{stage}/loss_R": losses["loss_R"].mean(),
        }
    ...
    def training_step(self, batch: Batch, batch_idx: int, **kwargs: dict[str, Any]) -> Any:
        ...
        try:
            ...
            # Real, uncurated data occasionally contains a degenerate complex
            # (e.g. a fragment with mass < 2, see STATUS.md PAUSE-PUNKT #14)
            # that the small hand-picked dummy set never exercised. Skip it
            # like an empty batch rather than crashing the whole run.
            print(f"[WARN] Skipping bad training batch at idx={batch_idx}: {e}")
            self.log("train/num_bad_batches", 1, on_step=True, on_epoch=True, sync_dist=True)
    ...
    def validation_step(self, batch: Batch, batch_idx: int, **kwargs: dict[str, Any]) -> Any:
        ...
        try:
            ...
            print(f"[WARN] Skipping bad validation batch at idx={batch_idx}: {e}")
            return None
        total_loss = loss_dict[f"loss_{stage}/total"]
