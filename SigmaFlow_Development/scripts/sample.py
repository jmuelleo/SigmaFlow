...
from sigmadock.chem.parsing import write_sdf
...
class SamplingModule(pl.LightningModule):
    ...
    def export_predictions_to_sdf(self, results: dict[str, list[dict[str, Any]]], out_dir: Path) -> None:
        """
        Write the final docked pose of every (complex, seed) result to its own sdf file
        Args: results:
        {results_key: [per_mol_out,...]} same dict save_results has <- example: "1G9V_RQ3::1G9V_RQ3_ligand"
        out_dir: directory to write the sdf files into
        """
        out_dir.mkdir(parents = True, exist_ok = True)
        for results_key, per_seed_list in results.items():
            for seed_idx, per_mol_out in enumerate(per_seed_list):
                mol = get_mol_from_coords(per_mol_out["x0_hat"], per_mol_out["lig_ref"])
                safe_name = results_key.replace("::","__")
                filename = f"{safe_name}_seed{seed_idx}.sdf"
                write_sdf([mol], out_dir/filename)
    ...
    def save_results(self, results: dict) -> None:
        ...
        self.export_predictions_to_sdf(results, out_dir)
